...
if 'admin_owner' not in st.session_state:
    st.session_state.admin_owner = None
...
def get_admin_owner():
    """Get the admin owner from the user database"""
    try:
        with open(USER_DB_FILE, 'r') as f:
            users = json.load(f)
        for username, details in users.items():
            if details.get('is_admin_owner', False):
                return username
        return None
    except:
        return None

def get_admin_csv_file():
    """Return the CSV file path specific to the admin owner"""
    admin_owner = st.session_state.admin_owner or get_admin_owner()
    if admin_owner:
        return f"fees_data_{admin_owner}.csv"
    return "fees_data_default.csv"

def get_admin_student_fees_file():
    """Return the student fees JSON file path specific to the admin owner"""
    admin_owner = st.session_state.admin_owner or get_admin_owner()
    if admin_owner:
        return f"student_fees_{admin_owner}.json"
    return "student_fees_default.json"
...
def initialize_student_fees():
    ...
    student_fees_file = get_admin_student_fees_file()
    ...
    try:
        ...
        if username in users:
            if verify_password(users[username]['password'], password):
                ...
                st.session_state.admin_owner = get_admin_owner()
...
def create_user(username, password, email, is_admin=False, is_admin_owner=False):
    ...
    try:
        ...
        # Only the first user can be admin owner if none exists
        if not any(user.get('is_admin_owner', False) for user in users.values()):
            is_admin_owner = is_admin  # First admin becomes admin owner
        else:
            is_admin_owner = False  # Subsequent users cannot be admin owners
...
def login_page():
    ...
    with tabs[0]:
        with st.form("signup_form"):
            ...
            # Only show admin checkbox if no admin owner exists
            with open(USER_DB_FILE, 'r') as f:
                users = json.load(f)
            is_admin_owner_exists = any(user.get('is_admin_owner', False) for user in users.values())
            
            is_admin = False
            if not is_admin_owner_exists:
                is_admin = st.checkbox("Register as Admin Owner (First admin user becomes owner)")
            ...
            if signup_submit:
                if not new_username or not new_password or not new_email:
                    ...
                    success, message = create_user(new_username, new_password, new_email, is_admin, is_admin)
                    if success:
        ...
        with st.form("create_user_form"):
            ...
            if submit:
                if not new_username or not new_password or not new_email:
                    ...
                    success, message = create_user(new_username, new_password, new_email, is_admin, is_admin_owner=False)
                    if success:
                        st.success(message)
            ...
            if menu == "View All Records":
                ...
                if df.empty:
                    ...
                    st.download_button(
                        ...
                        data=csv,
                        file_name=f"fee_records_{st.session_state.admin_owner or st.session_state.current_user}.csv",
                        mime="text/csv"
                    )
                    ...
                    for i, month in enumerate(MONTHS):
                        with tabs[i]:
                            ...
                            if not month_data.empty:
                                ...
                                #     data=csv,
                                #     file_name=f"{month.lower()}_payment_status_{st.session_state.admin_owner or st.session_state.current_user}.csv",
                                #     mime="text/csv"
                                # )
                            ...
                            #     label="📥 Download All Records as CSV",
                            #     data=csv,
                            #     file_name=f"all_fee_records_{st.session_state.admin_owner or st.session_state.current_user}.csv",
                            #     mime="text/csv",
                    ...
                    if not class_students:
                        ...
                        if student_data.empty:
                            ...
                            st.download_button(
                                ...
                                data=csv,
                                file_name=f"{selected_student}_fee_report_{st.session_state.admin_owner or st.session_state.current_user}.csv",
                                mime="text/csv"
                            )
